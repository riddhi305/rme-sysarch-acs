...
#define IOVIRT_NEXT_BLOCK(b) \
  (IOVIRT_BLOCK *)((uint8_t *)(&b->data_map[0]) + b->num_data_map * sizeof(NODE_DATA_MAP))
...
uint32_t pal_pcie_scan_bridge_devices_and_check_memtype(uint32_t seg, uint32_t bus, uint32_t dev,
                                                        uint32_t fn);
...
void pal_save_global_test_data(uint64_t nvm_mem, uint32_t total_tests, uint32_t tests_passed,
                               uint32_t tests_failed);
void pal_restore_global_test_data(uint64_t nvm_mem, uint32_t *total_tests, uint32_t *tests_passed,
                                  uint32_t *tests_failed);
