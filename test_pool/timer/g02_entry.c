...
/* 0 = mapping only; 1 = also arm CNTHPS (requires assembler & CPU support) */
...
/* Ask EL3 to set SCR_EL3.EEL2=1 and clear IRQ/FIQ/EA routing */
static 
int 
...
payload(void)
{
    ...
    val_print(ACS_PRINT_ALWAYS, " CurrentEL = 0x%lx ", val_pe_reg_read(CurrentEL));

    /* Must be running at Secure EL2 */
    if (val_pe_reg_read(CurrentEL) != AARCH64_EL2) {
        val_print(ACS_PRINT_ALWAYS,
                  " Skipping: requires Secure EL2 (CurrentEL=0x%lx) ",
                  val_pe_reg_read(CurrentEL));
        val_set_status(index, "SKIP", 1);
        return;
    }

    /* Ensure EL3 configuration is suitable for S-EL2 timer delivery */
    ...
    if (g_intid != 20U) {
        val_print(ACS_PRINT_ERR, " Expected INTID 20, platform reported %u ", g_intid);
        val_set_status(index, "FAIL", 4);
        return;
    }

#if TRY_ARM_CNTHPS
    /* Program & wait for the interrupt (requires FEAT_SEL2 and toolchain support) */
...
#else
    /* Mapping validated, don’t touch CNTHPS to avoid hangs on non-SEL2 toolchains/CPUs */
    val_print(ACS_PRINT_ALWAYS,
              " PASS: Platform publishes CNTHPS PPI=20; programming skipped ", 0);
